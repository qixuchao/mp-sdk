...
- 🐞 修复点击频次和展示频次清空本地消耗方广告位数据的逻辑
-  广告位配置调整，用V2版本的配置覆盖V1的配置
    - 将trackingV2Data从consumer级别提到slot级别
    - 配置中增加了屏蔽规则，H5用不到没什么影响
    - 频次优先级字段调整由`priorityPolicyPacingTarget`调整为`priorityPolicyPacingTargetV2`

## 1.18.0 
配置中增加广告位尺寸和比例，之后媒体广告位的配置可以直接读取线上配置
...
## 1.17.1
`2021-01-26`
- 🐞 修复设置频次控制时本地存储的数据结构不能同时存储点击和展现对应的频次数据
...
`2021-01-13`
- 🌟 增加广告的频次控制
- 🌟 调整adiMatch消耗方获取js链接的字段

## 1.15.1
`2021-01-09`
- 🐞 修复在消耗方竞选时，当有消耗方出错时会进入3s等待的逻辑
- 🐞 调整fancyapi和艾卖驰的广告样式
...
## 1.14.1
`2020-12-25`
- 🐞 修复广告容器未设置高度时，fancyapi广告展示素材被拉伸的问题
...
## 1.11.0
`2020-12-02`
- 🌟 配置化广告的可见性检测和动态计算消耗方的权重
- 🌟 点击时上报点击广告容器的尺寸，点击的位置，屏幕尺寸
...
- 🐞 处理广点通广告点击之后，会上报曝光的问题

## 1.9.3
`2020-11-18`

- 🐞 处理初始化时如果方法未执行完成，多次调用广点通请求的问题
