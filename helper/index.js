...
M$P_M_C.slotBiddings.forEach(function (slot) {
  slot.slotBidding.forEach(function (item) {
    item.consumer.appId = '1110655203';
    for (let key in item.trackingData) {
      item.trackingData[key] = item.trackingData[key].replace(
        /^http:/,
        'https:'
      );
    }
...
console.log(JSON.stringify(M$P_M_C.slotBiddings, null, 2));
